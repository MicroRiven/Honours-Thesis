...
def choose_atm_strike(opt: pd.DataFrame, expiry: pd.Timestamp, spot: float) -> float:
    sub = opt.loc[opt["expirDate"].eq(expiry), ["strike"]].drop_duplicates()
    if sub.empty:
        raise ValueError("No strikes for expiry")
...
def call_price(opt: pd.DataFrame, expiry: pd.Timestamp, strike: float) -> float | None:
    # Check if the row exists at all
    row = opt.loc[
        opt["expirDate"].eq(expiry) & opt["strike"].eq(strike)
    ]
    if row.empty:
        ticker = opt['ticker'].iloc[0] if not opt.empty else "unknown"
        logger.debug(f"No row found for {ticker} call at expiry={expiry.date()}, strike={strike}")
        return None
    c = row["cValue"].iloc[0]
    assert c >= 0.0, "Call price should be non-negative"
    if pd.isna(c):
        logger.debug(f"Call price is NaN for {opt['ticker'].iloc[0] if not opt.empty else 'unknown'} at expiry={expiry.date()}, strike={strike}")
        return None
    else:
        return c

def put_price(opt: pd.DataFrame, expiry: pd.Timestamp, strike: float) -> float | None:
    # Check if the row exists at all
    row = opt.loc[
        opt["expirDate"].eq(expiry) & opt["strike"].eq(strike)
    ]
    ...
    if row.empty:
        ticker = opt['ticker'].iloc[0] if not opt.empty else "unknown"
        logger.debug(f"No row found for {ticker} put at expiry={expiry.date()}, strike={strike}")
        return None
    p = row["pValue"].iloc[0]
    assert p >= 0.0, "Put price should be non-negative"
    if pd.isna(p):
        logger.debug(f"Put price is NaN for {opt['ticker'].iloc[0] if not opt.empty else 'unknown'} at expiry={expiry.date()}, strike={strike}")
        return None
    else:
        return p

def close_on_or_before(underlying_ticker: pd.DataFrame, date: pd.Timestamp) -> float:
    '''
    Returns the last available value ≤ date
    Returns NaN if date is before the first index value
    '''
    assert underlying_ticker.index.is_monotonic_increasing, "underlying_ticker must be indexed by trade_date and sorted"
    price = underlying_ticker["close"].asof(date)
    assert pd.notna(price), f"No available close price on or before {date.date()}"
    return float(price)
...
def open_positions_for_day(
    ...
    for ticker in tickers:
        ...
        short_c =  call_price(opt, short_exp, strike)
        short_p = put_price(opt, short_exp, strike)
        long_c = call_price(opt, long_exp, strike)
        long_p = put_price(opt, long_exp, strike)
...
def close_positions_for_day(
    ...
    for pos in open_positions:
        ...
        if opt.empty:
            logger.warning(f"Ticker {pos.ticker} not found in options data on {dt.date()} when closing position opened on {pos.open_date.date()}. Deleting position.")
            continue
        ...
        if pos.strategy == "short_straddle_hold":
            ...
            c = call_price(opt, pos.short_expiry, pos.strike)
            p = put_price(opt, pos.short_expiry, pos.strike)
            ...
            long_c = call_price(opt, pos.long_expiry, pos.strike)
            if long_c is None:
                logger.error(f"Missing option prices for {pos.strategy} {pos.ticker} opened on {pos.open_date.date()} when closing on {dt.date()}. Deleting position.")
                continue
            ...
            long_c = call_price(opt, pos.long_expiry, pos.strike)
            long_p = put_price(opt, pos.long_expiry, pos.strike)
            if None in [short_c, short_p, long_c, long_p]:
                logger.warning(f"Could not close position for {pos.strategy} {pos.ticker} opened on {pos.open_date.date()}: missing leg price on {dt.date()}.")
                continue
        ...
        if (notional is None) or (not np.isfinite(notional)) or (notional <= 0):
            logger.warning(f"Invalid notional for position {pos.ticker} opened on {pos.open_date.date()}, cannot compute normalized PnL.")
            continue
